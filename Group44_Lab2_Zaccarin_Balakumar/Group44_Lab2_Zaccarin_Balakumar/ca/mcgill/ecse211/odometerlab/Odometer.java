...
public class Odometer extends Thread {
	...
	public Odometer(EV3LargeRegulatedMotor leftMotor, EV3LargeRegulatedMotor rightMotor) {
		...
		this.x = -15.0; // change to 0 if counting first turn on X and Y
		this.y = -15.0; // change to 0 if counting first turn on X and Y
		this.theta = 0.0;
	...
	public void run() {
		...
		while (true) {
			...
			distL = 3.14159 * OdometryLab.WHEEL_RADIUS * (currentTachoL - lastTachoL)/180; //convert tacho count to distance travelled
			distR=3.14159*OdometryLab.WHEEL_RADIUS*(currentTachoR - lastTachoR) / 180; //convert tacho count to distance travelled
			lastTachoL=currentTachoL; //update tachometer count
			lastTachoR=currentTachoR; //update tachometer count
			deltaD= 0.5*(distL + distR);
			deltaT = (distL - distR)/OdometryLab.TRACK; // calculate variation in theta
