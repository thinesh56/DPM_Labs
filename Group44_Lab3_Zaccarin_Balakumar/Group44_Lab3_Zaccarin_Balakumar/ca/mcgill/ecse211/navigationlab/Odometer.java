...
public class Odometer extends Thread {
	...
	public Odometer(EV3LargeRegulatedMotor leftMotor, EV3LargeRegulatedMotor rightMotor) {
		...
		this.x = 0.0;
		this.y = 0.0;
		this.theta = 90.0; // north has the value of 90°
	...
	public void run() {
		...
		while (true) {
			...
			distL = 3.14159 * NavigationLab.WHEEL_RADIUS * (currentTachoL - lastTachoL)/180; //convert tacho count to distance travelled
			distR=3.14159*NavigationLab.WHEEL_RADIUS*(currentTachoR - lastTachoR) / 180; //convert tacho count to distance travelled
			lastTachoL=currentTachoL; //update tachometer count
			lastTachoR=currentTachoR; //update tachometer count
			deltaD= 0.5*(distL + distR);
			deltaT = (distL - distR)/NavigationLab.TRACK; // calculate variation in theta
			deltaT = Math.toDegrees(deltaT);
			//i changed DX, and DY
			dY= deltaD * Math.sin(Math.toRadians(fakeTheta)); // change in x and y according to robot orientation
			dX=deltaD * Math.cos(Math.toRadians(fakeTheta));
			synchronized (lock) {
				/**
